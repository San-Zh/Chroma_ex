// $Id: chroma.cc, version ex, 06/19/2024, Ke-Long$
/*! \file
...
bool linkageHack(void) {
    ...
    foo &= InlineWritePartPropEnv::registerAll();
...
int main(int argc, char *argv[]) {
    ...
    if (Chroma::getInputFileList().empty()) { Chroma::getInputFileList().push_back(Chroma::getXMLInputFileName()); }
    ...
    for (int list_index = 0; list_index < Chroma::getInputFileList().size(); list_index++) {
        ...
        if (list_index == 0) { push(xml_out, "chroma"); }
        ...
        try {
            ...
            Handle<GaugeInit> gaugeInit(TheGaugeInitFactory::Instance().createObject(input.cfg.id, cfgtop, input.cfg.path));
        ...
        QDPIO::cout << "CHROMA: Gauge field successfully read: time= " << swatch.getTimeInSeconds() << " secs" << std::endl;
        ...
        // Last?
        if (list_index == Chroma::getInputFileList().size() - 1) { pop(xml_out); }

        if (Chroma::getInputFileList().size() > 1) { TheNamedObjMap::Instance().erase_all(); }
